 ...
 * @date 2014年10月29日
 */
public class RollBackAssignee extends AbstractFoxBpmTestCase {
	/**
	 * 自动给指定处理者
	 */
	@Test
	@Deployment(resources = {"org/foxbpm/test/command/RollBack_assignee_1.bpmn"})
	public void testDesignatedHandler() {
		// 预置角色
		...
		expandTaskCommand.setProcessDefinitionKey("RollBack_assignee_1");
		...
		Task task = taskService.createTaskQuery().processDefinitionKey("RollBack_assignee_1").taskNotEnd().singleResult();
		...
		task = taskService.createTaskQuery().processDefinitionKey("RollBack_assignee_1").taskNotEnd().singleResult();
		expandTaskCommand = new ExpandTaskCommand();
		expandTaskCommand.setTaskCommandId("HandleCommand_1");
		expandTaskCommand.setCommandType("claim");
		expandTaskCommand.setTaskId(task.getId());
		...
		assertEquals("test03", task.getAssignee());
