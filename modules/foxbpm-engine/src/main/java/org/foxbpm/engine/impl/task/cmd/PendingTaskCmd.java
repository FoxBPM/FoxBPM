...
import org.foxbpm.engine.impl.task.command.PendingTaskCommand;
...
public class PendingTaskCmd extends AbstractExpandTaskCmd<PendingTaskCommand, Void> {
	...
	public PendingTaskCmd(PendingTaskCommand abstractCustomExpandTaskCommand) {
	...
	private void cloneAndInsertTask(TaskEntity task) {
		...
		newTask.setOwner(newTask.getAssignee());
		/** 重置任务非实例属性 */
		...
		/** 设置为转办状态 */
		newTask.setDelegationState(DelegationState.RESOLVED);
		/** 插入任务 */
