...
public class ActivityBehavior extends FlowNodeBehavior {
	...
	protected void loopExecute(FlowNodeExecutionContext executionContext) {
		...
		if (loopCharacteristics instanceof MultiInstanceLoopCharacteristics) {
			...
			String loopDataOutputCollectionExpressionValue = loopDataOutputCollection != null ? loopDataOutputCollection
			...
			if (loopDataInputCollectionExpressionValue != null && !loopDataInputCollectionExpressionValue.equals("")) {
				...
				try {
					...
					throw new FoxBPMExpressionException(ExceptionCode.EXPRESSION_EXCEPTION_LOOPDATAOUTPUTCOLLECTION_COLLECTIONCHECK,
							this.getId(), this.getName(), loopDataInputCollectionExpressionValue, e);
				...
				if (valueObj != null) {
					...
					if (valueObj instanceof Collection) {
						...
						if (valueObjCollection.size() == 0) {
							...
							throw new FoxBPMExpressionException(ExceptionCode.EXPRESSION_EXCEPTION_LOOPDATAINPUTCOLLECTIONEMPTY,
									this.getId(), this.getName(), loopDataInputCollectionExpressionValue);
						...
						for (Object object : valueObjCollection) {
							...
							try {
								...
								throw new FoxBPMExpressionException(ExceptionCode.EXPRESSIONEXCEPTION_COLLECTIONININPUTDATAITEM,
								...
								if (valueObjString.length > 0) {
									for (int i = 0; i < valueObjString.length; i++) {
										...
										try {
											...
											throw new FoxBPMExpressionException(
													ExceptionCode.EXPRESSIONEXCEPTION_COLLECTIONININPUTDATAITEM, this.getId(),
									...
									throw new FoxBPMExpressionException(
											ExceptionCode.EXPRESSION_EXCEPTION_LOOPDATAOUTPUTCOLLECTION_COLLECTIONCHECK, this.getId(),
	...
	public void leave(FlowNodeExecutionContext executionContext) {
		...
		if (loopCharacteristics != null) {
			...
			if (loopCharacteristics instanceof MultiInstanceLoopCharacteristics) {
				...
				// 多实例输入数据集
				Expression loopDataInputCollection = milc.getLoopDataInputCollection();
				String loopDataInputCollectionExpressionValue = loopDataInputCollection != null ? loopDataInputCollection
						.getExpressionText() : null;
