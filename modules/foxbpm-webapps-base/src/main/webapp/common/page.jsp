<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="org.foxbpm.web.common.util.Pagination"%>
<%
	Pagination pagination = null;
	Object tpagination = request.getAttribute("pageInfor");
	if (tpagination == null) {
		out.print("");
		return;
	} else {
	...
	if (pagination.getPageCount() > 1) {
		if (pagination.getPageIndex() == 1) {
			...
			sb.append(pagination.createPrePage(
					pagination.getPageIndex() - 1, false));
		}
	}
	// 当总页数小于8时显示全部页码
	...
	if (pagination.getPageCount() > 1) {
		if (pagination.getPageIndex() == pagination.getPageCount()) {
			...
			sb.append(pagination.createNextPage(
					pagination.getPageIndex() + 1, false));
		}
	}
