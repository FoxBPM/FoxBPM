...
<body>
	<form method="post" id="form" action="FlowManager">
		<div class="popup">
				...
				<c:forEach items="${result.dataList}" var="dataList"
					...
					<tr id="row${index.index}">
						<td><input type="checkbox" name="check"
							rowIndex="${index.index}" /></td>
						<td>${dataList.key}<input type="hidden"
							id="key${index.index}" value="${dataList.key}" /></td>
						<td></td>
						<td></td>
						<td></td>
						<td><input type="text" id="value${index.index}"
							value="${dataList.value}" /></td>
			...
			<table>
				<tr>
					<td><input type="hidden" id="nowIndex" value="${nowIndex+1}" />
						<input type="hidden" name="deleteIndex" id="deleteIndex" /> <input
						type="hidden" name="action" value="saveProcessVariables" /> <input
						type="hidden" name="insertAndUpdate" id="insertAndUpdate" /> <input
						type="hidden" name="processInstanceId"
						value="${result.processInstanceId}" />
						<div class="btn-normal">
							<a href="#" onclick="addNew();">新增</a>
						</div></td>
					<td>
						<div class="btn-normal">
							<a href="#" onclick="deleteRow();">删除</a>
						</div>
					</td>
					<td>
						<div class="btn-normal">
							<a href="#" onclick="saveData();">保存</a>
						</div>
					</td>
				</tr>
			</table>
